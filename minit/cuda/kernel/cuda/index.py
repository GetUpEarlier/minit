...
def generate_index_kernel(name: str):
...
__global__ void kernel(T* input, T* index, T* output, size_t a, size_t b, size_t c, size_t d) {
...
extern "C" void ${KERNEL_NAME}(cudaStream_t stream, T* input, T* index, T* output, size_t a, size_t b, size_t c, size_t d) {
    size_t nr_elements = a * c * d;
    if (nr_elements == 0) {
        return;
    }
